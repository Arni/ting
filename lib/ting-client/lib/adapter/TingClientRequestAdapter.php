...
//file_put_contents("/var/www/drupalvm/drupal/debug/searchtest1.txt", print_r($soapParameters, TRUE), FILE_APPEND);
        $client = new NanoSOAPClient($request->getWsdlUrl(), array('curl' => $curl_options));
        $response = $client->call($soapAction, $soapParameters);
//file_put_contents("/var/www/drupalvm/drupal/debug/searchtest2.txt", print_r($response, TRUE), FILE_APPEND);
